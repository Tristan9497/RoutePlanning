...
\begin{itemize}
	...
	\item few obstacles on right lane
	\item many obstacles on right lane
\end{itemize}
...
\textbf{No obstacles}\\
The reasoning for this test is, to check how good the robot stays on the right lane. Therefore the robot drives 5 rounds in the described test environment, while thee difference between the right lane and the robot center is monitored.
...
As pictured in \ref{noobserr} the robot has left the road markings twice. According to the analytics of the test node the first time for 2.77s and the second time for 0.36s.\\

\textbf{Obstacles on left lane}\\
This test is supposed to test the behavior of the navigation when obstacles block the view of the camera but the robot has to drive on the right side. In this test the robot will not drive as many rounds, since the goal is to observe the behavior when passing goals 

As visible in figure \ref{} the robot left the right lane way more often compared to \ref{noobserr}.\\
...
%This behavior can be slightly improved by tuning the parameters of the posefinder or the frequency of the costmaps.

\textbf{Few obstacles on right lane}\\

To cover the possible scenarios of the carolo cup this test is supposed to be a realistic application with a few obstacle scattered over the entire environment. Like in the last test the obstacles will be located as well in corners, as on straight section.
