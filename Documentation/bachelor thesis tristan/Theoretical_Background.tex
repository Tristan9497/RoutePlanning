...
\subsection{rqt}
rqt is a qt based tool for the development of GUI's (Graphical User Interface) for the ROS framework\cite{rqt}.
...
ROS offers the tools tf and tf2, with which for example sensor messages can be transformed from one tf frame in to another. Furthermore it offers the possibility to transform data back in time.
...
Obstacles in the costmap cannot only be marked, but also cleared by the subscribed data source. For each data source a configuration regarding the clearing and marking permissions is necessary.\\
...
\begin{itemize}
	...
	\item \textbf{Width} - max. 300 mm
	\item \textbf{Wheel base} - min. 200 mm
	\item \textbf{Steering} - at least one axle must be steerable
\end{itemize}

Track restrictions relevant in this thesis:
\begin{itemize}
	\item \textbf{lane width} 350-450 mm
	\item \textbf{sharpest turn radius} 1000 mm
\end{itemize}
